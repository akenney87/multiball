...
Testing the impact of reduced position bonuses on shot distribution.
"""
...
print("="*80)
print("PHASE 2 VALIDATION - 20 GAMES")
print("="*80)
print()
print("Testing reduced position bonuses:")
print("  Centers:        rim +25% -> +15%, 3pt -30% -> -25%, mid +5% -> +10%")
print("  Power Forwards: rim +10% -> +5%,  3pt -15% -> -10%, mid +5% (same)")
print()
...
print("="*80)
print("COMPARISON: BEFORE PHASE 1 -> PHASE 1 -> PHASE 2")
print("="*80)
print()
print(f"{'Shot Type':<15} {'Before P1':>12} {'Phase 1':>12} {'Phase 2':>12} {'NBA Target':>12} {'P1->P2 Change':>15}")
print("-"*80)
...
for shot_type in ['3pt', 'midrange', 'rim']:
    ...
    change_p1_to_p2 = current_pct - phase1

    print(f"{shot_type:<15} {before:>11.1f}% {phase1:>11.1f}% {current_pct:>11.1f}% {nba:>11.1f}% {change_p1_to_p2:>+14.1f}%")
...
# Calculate changes from Phase 1
rim_change = (shot_type_counter['rim'] / total_fga * 100) - phase1_results['rim']
mid_change = (shot_type_counter['midrange'] / total_fga * 100) - phase1_results['midrange']
three_change = (shot_type_counter['3pt'] / total_fga * 100) - phase1_results['3pt']

print(f"Rim attempts:     {phase1_results['rim']:.1f}% -> {(shot_type_counter['rim'] / total_fga * 100):.1f}% (change: {rim_change:+.1f}%)")
print(f"Midrange:         {phase1_results['midrange']:.1f}% -> {(shot_type_counter['midrange'] / total_fga * 100):.1f}% (change: {mid_change:+.1f}%)")
print(f"3PT:              {phase1_results['3pt']:.1f}% -> {(shot_type_counter['3pt'] / total_fga * 100):.1f}% (change: {three_change:+.1f}%)")
print()
...
if abs(rim_change) > 3:
    print("[OK] SIGNIFICANT IMPACT on rim attempts from Phase 2!")
elif abs(rim_change) > 1:
    print("[INFO] Moderate impact on rim attempts from Phase 2")
else:
    print("[WARN] Minimal impact on rim attempts from Phase 2")
...
if current_rim_pct <= 38:
    print("[OK] Rim attempts near target! Phase 1+2 successful.")
elif current_rim_pct <= 43:
    print("[INFO] Rim attempts improved but still 5-8% high. Phase 3 may help.")
else:
    print("[WARN] Rim attempts still significantly high. Phase 3-4 needed.")
...
if current_rim_pct <= 38 and current_mid_pct >= 20:
    print("[OK] Phase 1+2 achieved target distribution!")
    print("  -> Run full 100-game validation to confirm stability")
    print("  -> Continue to M4.5 FG% and PPG tuning")
elif current_rim_pct <= 43:
    print("[INFO] Phase 1+2 made good progress. Close to target.")
    print("  -> Consider Phase 3 (reduce player composite bonuses)")
    print("  -> Or run 100-game validation if results are acceptable")
else:
    print("[WARN] Phase 1+2 insufficient. Additional phases needed.")
    print("  -> Implement Phase 3 (reduce player composite bonuses +32% -> +20%)")
    print("  -> May need Phase 4-5 for full correction")
