...
SIGMOID_K = 0.025  # Tuned for attribute correlation (1.67x signal increase from 0.015)
...
BASE_RATE_3PT = 0.28              # 3-point shot (wide open) - M4.5 PPG Fix: Increased from 0.23→0.28 to hit 36-37% league average
BASE_RATE_MIDRANGE_SHORT = 0.50   # 10-16 ft (open) - Tuned for ~40% midrange%
BASE_RATE_MIDRANGE_LONG = 0.41    # 16-23 ft (open) - Tuned for ~35% midrange%
BASE_RATE_DUNK = 0.87             # Dunk (uncontested) - M5.0: Reduced from 0.95→0.87 for better NBA realism
BASE_RATE_LAYUP = 0.62            # Layup (contested baseline) - M5.0: Reduced from 0.77→0.62 for better NBA realism
BASE_RATE_FREE_THROW = 0.50       # Free throw - Will be adjusted separately in free_throws.py
...
WEIGHTS_LAYUP = {
    'finesse': 0.30,                    # -0.05 - touch and body control
    ...
    'core_strength': 0.10,              # NEW - finishing through contact at rim
...
WEIGHTS_REBOUND = {
    'height': 0.20,                     # -0.02 - reach advantage
    ...
    'grip_strength': 0.10,              # UNCHANGED - securing the ball
...
WEIGHTS_CONTEST = {
    ...
    'determination': 0.10,              # NEW - hustle, effort to contest
}
...
WEIGHTS_STEAL_DEFENSE = {
    'grip_strength': 0.30,              # Most important - ability to grab/strip ball
    'reactions': 0.25,                  # Quick hands and anticipation
    'awareness': 0.20,                  # Reading passes, positioning
    'agility': 0.15,                    # Quick movement to intercept
    'determination': 0.10,              # Hustle and effort
}
...
WEIGHTS_TURNOVER_PREVENTION = {
    ...
    'throw_accuracy': 0.20,         # NEW - prevents bad passes
...
WEIGHTS_BALL_HANDLING = {
    'hand_eye_coordination': 0.30,  # Dribbling coordination
    'awareness': 0.25,              # Court vision to run offense
    'agility': 0.20,                # Ability to handle with quick movements
    'composure': 0.15,              # Decision-making with ball
    'grip_strength': 0.10,          # Ball security
}
...
WEIGHTS_DRIVE_LAYUP = {
    ...
    'acceleration': 0.10,               # UNCHANGED - first-step burst critical
    'core_strength': 0.10,              # NEW - absorbing contact on drives
...
WEIGHTS_DRIVE_KICKOUT = {
    ...
    'awareness': 0.30,                  # -0.05 - finding open shooters
    'composure': 0.20,                  # -0.05 - decision-making under pressure
...
WEIGHTS_DRIVE_TURNOVER = {
    ...
    'throw_accuracy': 0.20,          # NEW - accurate kickout passes
...
WEIGHTS_TRANSITION_SUCCESS = {
    ...
    'composure': 0.10,              # Decision-making at speed (unchanged)
...
WEIGHTS_SHOT_SEPARATION = {
    ...
    'acceleration': 0.20,           # First step
    'creativity': 0.10,             # Unconventional moves
    'composure': 0.10,              # Patience to wait
}
...
WEIGHTS_FIND_OPEN_TEAMMATE = {
    ...
    'teamwork': 0.20,                   # UNCHANGED - willingness to share
    'throw_accuracy': 0.15,             # NEW - delivering accurate passes
    'composure': 0.10,                  # -0.02 - decision-making under pressure
...
ASSIST_CONCENTRATION_EXPONENT = 4.0
...
WEIGHTS_HELP_DEFENSE_ROTATION = {
    'teamwork': 0.30,               # Team-first mindset
    'awareness': 0.30,              # Reading help situations
    'reactions': 0.20,              # Quick rotation
...
PATIENCE_CONTEST_REDUCTION_SCALE = 0.001  # Max ±5% adjustment at extremes
...
PATIENCE_DISTANCE_MODIFIER_SCALE = 0.02  # ±0.8ft at ±40 patience difference
...
CONTEST_DISTANCE_SIGMA = 1.9  # Standard deviation in feet for Gaussian variance
...
CONSISTENCY_VARIANCE_SCALE = 0.0002  # ±0.8% at consistency=90, ±8% at consistency=10
...
SHOT_DISTRIBUTION_PLAYER_MOD = 0.15

# Tactical modifiers (±10%)
SHOT_DISTRIBUTION_TACTICAL_MOD = 0.10

# Zone defense bonus to 3PT attempts
ZONE_DEFENSE_3PT_ATTEMPT_BONUS = 0.05
...
USAGE_SCORING_OPTION_1 = 0.30
USAGE_SCORING_OPTION_2 = 0.20
USAGE_SCORING_OPTION_3 = 0.15
USAGE_OTHERS = 0.35  # Divided equally among remaining players
...
CONTEST_DISTANCE_WIDE_OPEN = 6.0      # feet
CONTEST_DISTANCE_CONTESTED = 2.0      # feet
...
CONTEST_DEFENDER_MOD_SCALE = 0.0005   # OPTION A: -50% reduction from 0.001 → (composite - 50) * 0.0005 = ±0.025
...
HELP_DEFENSE_TRIGGER_THRESHOLD = 0.30  # Contest quality < 30%
HELP_DEFENSE_AWARENESS_K = 0.05        # Sigmoid steepness for help rotation
...
TRANSITION_BONUS_RIM = 0.20       # +20% success
TRANSITION_BONUS_MIDRANGE = 0.12  # +12% success
TRANSITION_BONUS_3PT = 0.08       # +8% success
...
ZONE_DEFENSE_CONTEST_PENALTY = -0.15   # -15% contest effectiveness on 3PT
ZONE_DEFENSE_DRIVE_PENALTY = -0.10     # -10% drive success
...
PACE_FAST_POSSESSION_MOD = 1.10       # +10% possessions
PACE_FAST_STAMINA_DRAIN = 1.15        # +15% stamina drain
PACE_SLOW_POSSESSION_MOD = 0.90       # -10% possessions
PACE_SLOW_STAMINA_DRAIN = 0.85        # -15% stamina drain
PACE_STANDARD_POSSESSION_MOD = 1.0
PACE_STANDARD_STAMINA_DRAIN = 1.0
...
BASE_TURNOVER_RATE = 0.08             # 8% base rate for general possession turnovers (drive adds more)
...
TURNOVER_TYPE_BAD_PASS = 0.40
TURNOVER_TYPE_LOST_BALL = 0.30
TURNOVER_TYPE_OFFENSIVE_FOUL = 0.20
TURNOVER_TYPE_SHOT_CLOCK = 0.05         # NEW - shot clock violations
TURNOVER_TYPE_OTHER_VIOLATION = 0.05    # Traveling, carries, etc.

# Shot clock violation pace modifiers
SHOT_CLOCK_VIOLATION_SLOW_PACE_BONUS = 0.03  # +3% for slow pace
SHOT_CLOCK_VIOLATION_FAST_PACE_PENALTY = -0.02  # -2% for fast pace

# =============================================================================
# REBOUNDING STRATEGY (Section 10.4)
# =============================================================================

REBOUND_STRATEGY_CRASH_GLASS_COUNT = 5
REBOUND_STRATEGY_STANDARD_COUNT = 4
REBOUND_STRATEGY_PREVENT_TRANSITION_COUNT = 3

# Defensive rebounding advantage (Section 7.1)
DEFENSIVE_REBOUND_ADVANTAGE = 1.15    # 15% stronger
...
STAMINA_THRESHOLD = 80                # Below this, attributes degrade
STAMINA_DEGRADATION_POWER = 1.3       # Exponential curve exponent
STAMINA_DEGRADATION_SCALE = 0.002     # penalty = 0.002 * (80 - stamina) ** 1.3 (decimal format)

# Stamina costs per possession (Section 11.2)
STAMINA_COST_PER_POSSESSION_FAST = 1.5
STAMINA_COST_PER_POSSESSION_STANDARD = 1.3
STAMINA_COST_PER_POSSESSION_SLOW = 1.1

# Stamina costs per action
STAMINA_COST_DRIVE = 1.0
STAMINA_COST_JUMP_SHOT = 0.5
STAMINA_COST_REBOUND = 0.8
STAMINA_COST_SPRINT_TRANSITION = 1.2

# Recovery per minute on bench (Section 11.3)
STAMINA_RECOVERY_RATE = 8             # 8 * (1 - current/100)
...
STAMINA_RECOVERY_RATE_MAX_MODIFIER = 0.13  # ±13% recovery rate at extremes
...
BLOCK_DISTANCE_THRESHOLD_FAR = 6.0    # 6+ feet: cannot block (same as wide open threshold)
BLOCK_DISTANCE_THRESHOLD_MID = 3.0    # 3-6 feet: reduced block chance (50%)
# < 3 feet: full block chance (100%)

# Defender block ability weights
WEIGHTS_BLOCK_DEFENDER = {
    'jumping': 0.25,                  # Primary - reach and timing
    'height': 0.20,                   # Taller players block better
    'reactions': 0.15,                # Timing the jump
    'agility': 0.12,                  # Lateral movement to position
    'awareness': 0.10,                # Reading the shot
...
BLOCK_OUTCOME_STAYS_IN_PLAY = 0.65    # 65% - becomes loose ball/rebound
BLOCK_OUTCOME_OUT_OFF_SHOOTER = 0.23  # 23% - out of bounds, defense possession
BLOCK_OUTCOME_OUT_OFF_BLOCKER = 0.12  # 12% - out of bounds, offense retains

# Defender's ability to control blocked shot (controlled deflection vs wild swat)
WEIGHTS_BLOCK_CONTROL = {
    'grip_strength': 0.30,            # Hold onto deflected ball
    'hand_eye_coordination': 0.25,    # Clean contact
    'reactions': 0.20,                # Quick hands
    'composure': 0.15,                # Controlled motion
    'awareness': 0.10                 # Know where ball is going
}
...
WEIGHTS_BLOCK_SHOOTER_RECOVER = {
    ...
    'composure': 0.20,                # Don't panic
    'awareness': 0.10,                # See block coming
    'agility': 0.10                   # Body control
}
...
WEIGHTS_OUT_OFF_SHOOTER = {
    ...
    'arm_strength': 0.30,             # Power through shot
    'height': 0.20,                   # Reach advantage
...
WEIGHTS_OUT_OFF_BLOCKER = {
    'finesse': 0.30,                  # Soft touch to redirect
    'hand_eye_coordination': 0.30,    # Quick hands
    'awareness': 0.20,                # See opportunity
    'deception': 0.20                 # Misdirect defender
}
...
OREB_PUTBACK_HEIGHT_THRESHOLD = 75    # Height > 75 → putback attempt
OREB_SHOT_CLOCK_RESET = 14            # Seconds
...
ASSIST_TIME_THRESHOLD = 2.0           # seconds since last pass
...
ATTRIBUTE_MIN = 1
ATTRIBUTE_MAX = 100
MINUTES_ALLOTMENT_TOTAL = 240         # 5 players × 48 minutes
MINUTES_ALLOTMENT_MAX_PER_PLAYER = 48
